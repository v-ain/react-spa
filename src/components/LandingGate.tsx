...
export default function LandingGate({ onInitializationComplete }: LandingGateProps) {
  ...
  return (
    <div style={{ display: 'flex', width: '100vw', height: '100vh', justifyContent: 'center', alignItems: 'center', backgroundColor: 'var(--bg-matrix)', position: 'relative' }}>
      <div style={{ position: 'absolute', width: '500px', height: '500px', background: 'radial-gradient(circle, rgba(56,189,248,0.06) 0%, transparent 70%)', pointerEvents: 'none' }} />

      <div style={{ width: '400px', backgroundColor: 'var(--bg-panel)', border: '1px solid var(--matrix-border)', borderRadius: '8px', padding: '32px', boxShadow: '0 20px 50px rgba(0,0,0,0.6)' }}>
        ...
        <form onSubmit={(e) => { e.preventDefault(); onInitializationComplete(); }}>
          <div style={{ marginBottom: '16px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
            ...
            <input type="text" value={cryptoToken} disabled readOnly style={{ backgroundColor: 'var(--bg-terminal)', border: '1px solid var(--matrix-border)', color: 'var(--text-bright)', padding: '10px', borderRadius: '4px', fontFamily: 'monospace' }} />
          </div>
          ...
          <div style={{ marginBottom: '24px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
            ...
            <select value={clusterNode} onChange={(e) => setClusterNode(e.target.value)} style={{ backgroundColor: 'var(--bg-terminal)', border: '1px solid var(--matrix-border)', color: 'var(--text-bright)', padding: '10px', borderRadius: '4px', outline: 'none' }}>
              <option value="node_eu#401">EU_MAIN_CLUSTER_NODE</option>
              <option value="node_us#102">US_BACKUP_CLUSTER_NODE</option>
            </select>
          </div>

          <button type="submit" disabled={bruteForceActive} style={{ width: '100%', padding: '12px', backgroundColor: 'var(--text-neon)', color: '#030407', border: 'none', borderRadius: '4px', fontWeight: 600, cursor: 'pointer', fontFamily: 'monospace' }}>
