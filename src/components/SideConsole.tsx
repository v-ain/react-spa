...
export default function SideConsole({ modules, activeId, onModuleSelect, filter, onFilterChange }: SideConsoleProps) {
  return (
    ...
    >
      ...
      <div
        style={{ padding: '0 16px 14px 16px', borderBottom: '1px solid var(--matrix-border)', position: 'relative' }}
      >
        <input
          type="text"
          value={filter}
        ...
        {modules.map((mod) => (
          <li
            ...
            style={{
              ...
              borderRadius: '6px',
              cursor: 'pointer',
              borderLeft: activeId === mod.id ? '3px solid var(--text-neon)' : '3px solid transparent',
              backgroundColor: activeId === mod.id ? 'var(--bg-active)' : 'transparent',
              transition: 'all 0.15s ease',
            }}
          >
            <span style={{ color: 'var(--text-neon)', fontSize: '11px', marginRight: '10px', marginTop: '2px' }}>
              {mod.prefix}
            </span>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <span
                    ...
                    style={{
                      ...
                      height: '6px',
                      borderRadius: '50%',
                      backgroundColor: mod.status === 'online' ? 'var(--cyber-green)' : 'var(--cyber-amber)',
                    }}
                  />
